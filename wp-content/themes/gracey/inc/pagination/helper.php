...
if ( ! function_exists( 'gracey_add_rest_api_pagination_global_variables' ) ) {
	/**
	 ...
	 * @return array
	 */
	function gracey_add_rest_api_pagination_global_variables( $global, $namespace ) {
		$global['paginationRestRoute'] = $namespace . '/get-posts';

		return $global;
	}

	add_filter( 'gracey_filter_rest_api_global_variables', 'gracey_add_rest_api_pagination_global_variables', 10, 2 );
}

if ( ! function_exists( 'gracey_add_rest_api_pagination_route' ) ) {
	/**
	 ...
	 * @return array
	 */
	function gracey_add_rest_api_pagination_route( $routes ) {
		$routes['pagination'] = array(
			'route'    => 'get-posts',
			'methods'  => WP_REST_Server::READABLE,
			'callback' => 'gracey_get_new_posts',
			'args'     => array(
				'options' => array(
					'required'          => true,
					...
					'description'       => esc_html__( 'Options data is array with all selected shortcode parameters value', 'gracey' ),
				),
			),
		);
	...
	add_filter( 'gracey_filter_rest_api_routes', 'gracey_add_rest_api_pagination_route' );
}

if ( ! function_exists( 'gracey_get_new_posts' ) ) {
	/**
	 * Function that load new posts for pagination functionality
	 *
	 * @return void
	 */
	function gracey_get_new_posts() {

		if ( ! isset( $_GET ) || empty( $_GET ) ) {
			gracey_get_ajax_status( 'error', esc_html__( 'Get method is invalid', 'gracey' ) );
		} else {
			...
			if ( ! empty( $options ) ) {
				...
				$query_args = gracey_get_query_params( $options );
				...
				echo apply_filters( "gracey_filter_{$get_template_part}", $get_template_part( $module . '/' . $shortcode, 'templates/loop', '', $options ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
				...
				if ( 'standard' === $options['pagination_type'] ) {
					$pagination_html = gracey_core_get_theme_template_part( 'pagination', 'templates/pagination', $options['pagination_type'], $options );
				} else {
					$pagination_html = '';
				}

				$data = apply_filters(
					'gracey_filter_pagination_data_return',
					array(
						'html'            => $html,
				...
				gracey_get_ajax_status( 'success', esc_html__( 'Items are loaded', 'gracey' ), $data );
			} else {
				gracey_get_ajax_status( 'error', esc_html__( 'Options are invalid', 'gracey' ) );
			}
		}
	}
}

if ( ! function_exists( 'gracey_get_query_params' ) ) {
	/**
	 ...
	 * @return array
	 */
	function gracey_get_query_params( $params ) {
		...
		return apply_filters( 'gracey_filter_query_params', $args, $params );
	}
}

if ( ! function_exists( 'gracey_get_pagination_data' ) ) {
	/**
	 ...
	 * @return array
	 */
	function gracey_get_pagination_data( $plugin, $module, $shortcode, $post_type, $params ) {
		$data = array();
		...
		if ( ! empty( $post_type ) && ! empty( $params ) ) {
			...
			if ( isset( $params['space'] ) && ! empty( $params['space'] ) ) {
				$params['space_value'] = gracey_get_space_value( $params['space'] );
			}
...
if ( ! function_exists( 'gracey_add_link_pages_after_content' ) ) {
	/**
	 * Function which add pagination for blog single and page
	 */
	function gracey_add_link_pages_after_content() {

		$args_pages = array(
			'before'      => '<div class="qodef-single-links qodef-m"><span class="qodef-m-single-links-title">' . esc_html__( 'Pages: ', 'gracey' ) . '</span>',
			'after'       => '</div>',
			'link_before' => '<span>',
			'link_after'  => '</span>',
			'pagelink'    => '%',
	...
	add_action( 'gracey_action_after_blog_single_content', 'gracey_add_link_pages_after_content' );
	add_action( 'gracey_action_after_page_content', 'gracey_add_link_pages_after_content' );
}

...
if ( ! class_exists( 'Gracey_Welcome_Page' ) ) {
	class Gracey_Welcome_Page {
		private static $instance;

		/**
		 * get the instance of Gracey_Welcome_Page
		 *
		 * @return self
		 */
		...
		function init_activation_hook() {
			if ( ! is_network_admin() ) {
				set_transient( '_gracey_welcome_page_redirect', 1, 30 );
			}
		}
		...
		function welcome_page_redirect() {
			...
			if ( ! get_transient( '_gracey_welcome_page_redirect' ) ) {
				return;
			}

			// delete the redirect transient
			delete_transient( '_gracey_welcome_page_redirect' );
			...
			wp_safe_redirect( add_query_arg( array( 'page' => 'gracey_welcome_page' ), esc_url( admin_url( 'themes.php' ) ) ) );
			exit;
		}
		...
		function create_welcome_page() {
			...
			remove_submenu_page( 'themes.php', 'gracey_welcome_page' );
		}
		...
		function welcome_page_content() {
			...
			$params['theme_screenshot']  = file_exists( GRACEY_ROOT_DIR . '/screenshot.png' ) ? GRACEY_ROOT . '/screenshot.png' : GRACEY_ROOT . '/screenshot.jpg';

			gracey_template_part( 'welcome', 'templates/welcome', '', $params );
		}
		...
		function enqueue_styles( $hook ) {
			...
			if ( 'appearance_page_gracey_welcome_page' === $hook ) {
				wp_enqueue_style( 'gracey-welcome-page-style', GRACEY_INC_ROOT . '/welcome/assets/admin/css/welcome.min.css' );
			}
		}
	}

	Gracey_Welcome_Page::get_instance();
}

	...
	var qodefJustifiedGallery = {
		...
		setJustifyGallery: function ( $holder ) {
			...
			qodef.qodefWaitForImages.check(
				$galleryInner,
				function () {
					if ( typeof $galleryInner.justifiedGallery === 'function' && qodefCore.windowWidth > 480 ) {
		...
		initLoadMore: function ( $holder ) {
			...
			$loadMoreButton.on(
				...
				function ( e ) {
					...
					if( ! $holder.hasClass('qodef--loading') ) {
		...
		loadMoreButtonVisibility: function ( $holder, options ) {
			if ( $holder.hasClass( 'qodef-pagination-type--load-more' ) ) {
				...
				if ( options.next_page > options.max_pages_num || options.max_pages_num === 1 ) {
					...
					$holder.find( '.qodef--load-more' ).hide();
...
(function ( $ ) {
	 ...
	 */
	var qodefWooModifiedButton = {
		init: function () {
			var $button = $('.woocommerce-form-login__submit, .coupon button, button[name="update_cart"], .checkout-button, #place_order, .wc-forward, ' +
				'.woocommerce-Button, .wc-backward, .qodef-woo-product-image-inner a, .single_add_to_cart_button, .woocommerce-form-coupon button,.wc-forward,' +
				'.wc-backward, .qodef-m-checkout-link, .qodef-m-cart-link, .qodef-woo-product-button-holder a, .qodef-woo-product-content .button, .price_slider_wrapper .button, .qodef-woo-dropdown-cart  .qodef-m-action-link');
			// var $button = $('.single_add_to_cart_button, .woocommerce-message .wc-forward.button,.woocommerce-cart .actions .button,' +
			// 	'.woocommerce-cart .wc-proceed-to-checkout .button,.woocommerce-checkout .button#place_order');
			qodefWooModifiedButton.addHtml($button);
		},
		
		addHtml: function (button){
			var $icon = '<svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="28" height="18" viewBox="0 0 28 18" xml:space="preserve"><path d="M27.6,8.5c-5.5,0-9.5-8.1-9.5-8.2C17.9,0,17.7,0,17.5,0.1c-0.2,0.1-0.3,0.4-0.1,0.7c0.1,0.3,2.7,5.5,6.7,7.7H0.4C0.2,8.5,0,8.7,0,9c0,0.3,0.2,0.5,0.4,0.5h23.7c-4,2.2-6.6,7.4-6.7,7.7c-0.1,0.2,0,0.5,0.1,0.7c0.2,0.1,0.4,0.1,0.6-0.2c0-0.1,4.1-8.2,9.5-8.2C27.8,9.5,28,9.3,28,9C28,8.7,27.8,8.5,27.6,8.5z"></path><path d="M27.6,8.5c-5.5,0-9.5-8.1-9.5-8.2C17.9,0,17.7,0,17.5,0.1c-0.2,0.1-0.3,0.4-0.1,0.7c0.1,0.3,2.7,5.5,6.7,7.7H0.4C0.2,8.5,0,8.7,0,9c0,0.3,0.2,0.5,0.4,0.5h23.7c-4,2.2-6.6,7.4-6.7,7.7c-0.1,0.2,0,0.5,0.1,0.7c0.2,0.1,0.4,0.1,0.6-0.2c0-0.1,4.1-8.2,9.5-8.2C27.8,9.5,28,9.3,28,9C28,8.7,27.8,8.5,27.6,8.5z"></path></svg>';
			if (button.length) {
				button.each(function () {
					var thisButton = $(this);
					
					if (!thisButton.find('.qodef-btn-icon').length) {
						thisButton.addClass('qodef-theme-button qodef-button');
						thisButton.wrapInner( "<div class='qodef-m-text'></div>" );
						thisButton.prepend('<span class="qodef-btn-icon">' + $icon + '</span>');
					}
				});
			}
		}
	...
	qodef.qodefWooModifiedButton = qodefWooModifiedButton;

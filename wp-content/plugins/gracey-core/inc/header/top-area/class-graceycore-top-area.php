...
class GraceyCore_Top_Area {
	...
	public function __construct() {
		...
		add_action( 'gracey_action_before_page_header', array( $this, 'load_template' ) );
		...
		add_filter( 'gracey_filter_add_inline_style', array( $this, 'set_inline_top_area_styles' ) );
		add_filter( 'gracey_core_filter_top_area_inner_class', array( $this, 'set_grid_class' ) );
	}

	/**
	 * @return GraceyCore_Top_Area
	 */
	...
	function is_top_area_enabled() {
		$top_area_enabled = 'yes' === gracey_core_get_post_value_through_levels( 'qodef_top_area_header' );
	...
	public function set_top_area_height() {
		if ( $this->is_top_area_enabled ) {
			$height         = gracey_core_get_post_value_through_levels( 'qodef_top_area_header_height' );
	...
	public function is_top_area_transparent() {
		$background_color = gracey_core_get_post_value_through_levels( 'qodef_top_area_header_background_color' );
	...
	public function load_template() {
		...
		gracey_core_template_part( 'header/top-area/', 'templates/top-area', '', $parameters );
	}
	...
	public function set_inline_top_area_styles( $style ) {
		...
		$height       = gracey_core_get_post_value_through_levels( 'qodef_top_area_header_height' );
		$side_padding = gracey_core_get_post_value_through_levels( 'qodef_top_area_header_side_padding' );
	...
	public function set_grid_class( $class ) {
		$class .= 'yes' === gracey_core_get_post_value_through_levels( 'qodef_top_area_header_in_grid' ) ? 'qodef-content-grid' : '';

		return $class;
	}
}

GraceyCore_Top_Area::get_instance();

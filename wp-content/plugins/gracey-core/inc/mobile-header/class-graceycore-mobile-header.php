...
abstract class GraceyCore_Mobile_Header {
	...
	public function __construct() {
		...
		add_filter( 'gracey_filter_add_inline_style', array( $this, 'set_inline_mobile_header_styles' ) );
		add_filter( 'gracey_core_filter_content_margin_mobile', array( $this, 'get_content_margin' ) );
		add_filter( 'gracey_core_filter_title_padding_mobile', array( $this, 'get_title_padding' ) );
	...
	public function load_template( $parameters = array() ) {
		$parameters = apply_filters( 'gracey_core_filter_mobile_header_template', $parameters );

		return gracey_core_get_template_part( 'mobile-header/layouts/' . $this->get_layout(), 'templates/' . $this->get_layout(), $this->get_layout_slug(), $parameters );
	}

	public function set_inline_mobile_header_styles( $style ) {
		$item_styles = array();

		$height           = gracey_core_get_post_value_through_levels( 'qodef_' . $this->get_layout() . '_mobile_header_height' );
		$background_color = gracey_core_get_post_value_through_levels( 'qodef_' . $this->get_layout() . '_mobile_header_background_color' );
		...
		$side_padding = gracey_core_get_post_value_through_levels( 'qodef_' . $this->get_layout() . '_mobile_header_side_padding' );
	...
	function set_header_height() {
		$header_height = gracey_core_get_post_value_through_levels( 'qodef_' . $this->get_layout() . '_mobile_header_height' );
		$header_height = ! empty( $header_height ) ? intval( $header_height ) : $this->default_header_height;

		$this->header_height = apply_filters( 'gracey_core_filter_set_mobile_header_height', $header_height );
	}

...
if ( ! function_exists( 'gracey_core_add_portfolio_single_sidebar_meta_boxes' ) ) {
	/**
	 * Function that add general meta box options for this module
	 *
	 * @param object $page
	 */
	function gracey_core_add_portfolio_single_sidebar_meta_boxes( $page ) {
		...
		if ( $page ) {
			...
			$sidebar_tab = $page->add_tab_element(
				array(
					...
					'description' => esc_html__( 'Portfolio sidebar settings', 'gracey-core' ),
				)
			);
	...
	add_action( 'gracey_core_action_after_portfolio_meta_box_map', 'gracey_core_add_portfolio_single_sidebar_meta_boxes', 12 );
}

...
if ( ! function_exists( 'gracey_core_add_page_footer_meta_box' ) ) {
	/**
	 * Function that add general meta box options for this module
	 *
	 * @param object $page
	 */
	function gracey_core_add_page_footer_meta_box( $page ) {

		if ( $page ) {
			$custom_sidebars = gracey_core_get_custom_sidebars();
			$footer_columns  = apply_filters( 'gracey_core_filter_footer_areas_columns_size', array() );

			$footer_tab = $page->add_tab_element(
				array(
					'name'        => 'tab-footer',
					'icon'        => 'fa fa-cog',
					'title'       => esc_html__( 'Footer Settings', 'gracey-core' ),
					'description' => esc_html__( 'Footer layout settings', 'gracey-core' ),
				)
			);
				...
				for ( $i = 1; $i <= intval( $footer_columns['footer_top_sidebars_number'] ); $i ++ ) {
					$top_footer_area_section->add_field_element(
						array(
							...
							'title'       => sprintf( esc_html__( 'Custom Footer Top Area - Column %s', 'gracey-core' ), $i ),
							'description' => sprintf( esc_html__( 'Widgets added here will appear in the %s column of top footer area', 'gracey-core' ), $i ),
				...
				for ( $i = 1; $i <= intval( $footer_columns['footer_bottom_sidebars_number'] ); $i ++ ) {
					$bottom_footer_area_section->add_field_element(
						array(
							...
							'title'       => sprintf( esc_html__( 'Custom Footer Bottom Area - Column %s', 'gracey-core' ), $i ),
							'description' => sprintf( esc_html__( 'Widgets added here will appear in the %s column of bottom footer area', 'gracey-core' ), $i ),
							'options'     => $custom_sidebars,
						)
					);
				}
			...
			do_action( 'gracey_core_action_after_page_footer_meta_box_map', $footer_tab );
		}
	}

	add_action( 'gracey_core_action_after_general_meta_box_map', 'gracey_core_add_page_footer_meta_box' );
}

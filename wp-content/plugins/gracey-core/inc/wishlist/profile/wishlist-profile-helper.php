...
if ( ! function_exists( 'gracey_core_add_wishlist_profile_navigation_item' ) ) {
	function gracey_core_add_wishlist_profile_navigation_item( $items, $dashboard_url ) {
		$user = wp_get_current_user();

		$items['my-wishlist'] = array(
			'url'         => esc_url( add_query_arg( array( 'user-action' => 'my-wishlist' ), $dashboard_url ) ),
			'text'        => esc_html__( 'My Wishlist', 'gracey-core' ),
	...
	add_filter( 'gracey_membership_filter_dashboard_navigation_pages', 'gracey_core_add_wishlist_profile_navigation_item', 10, 2 );
}

if ( ! function_exists( 'gracey_core_add_wishlist_profile_navigation_pages' ) ) {
	function gracey_core_add_wishlist_profile_navigation_pages( $html, $action ) {

		if ( 'my-wishlist' === $action ) {
			$atts = array();

			$html = gracey_core_get_template_part( 'wishlist', '/profile/templates/my-wishlist', '', $atts );
		}

		return $html;
	}

	add_filter( 'gracey_membership_filter_dashboard_page', 'gracey_core_add_wishlist_profile_navigation_pages', 10, 2 );
}

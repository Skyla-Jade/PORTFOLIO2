...
if ( ! function_exists( 'gracey_core_add_separator_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function gracey_core_add_separator_shortcode( $shortcodes ) {
		$shortcodes[] = 'GraceyCore_Separator_Shortcode';

		return $shortcodes;
	}

	add_filter( 'gracey_core_filter_register_shortcodes', 'gracey_core_add_separator_shortcode', 9 );
}

if ( class_exists( 'GraceyCore_Shortcode' ) ) {
	class GraceyCore_Separator_Shortcode extends GraceyCore_Shortcode {

		public function map_shortcode() {
			$this->set_shortcode_path( GRACEY_CORE_SHORTCODES_URL_PATH . '/separator' );
			$this->set_base( 'gracey_core_separator' );
			$this->set_name( esc_html__( 'Separator', 'gracey-core' ) );
			$this->set_description( esc_html__( 'Shortcode that displays separator with provided parameters', 'gracey-core' ) );
			...
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'width',
					'title'      => esc_html__( 'Width (px or %)', 'gracey-core' ),
					'group'      => esc_html__( 'Style', 'gracey-core' ),
				)
			);
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'thickness',
					'title'      => esc_html__( 'Thickness (px)', 'gracey-core' ),
					'group'      => esc_html__( 'Style', 'gracey-core' ),
				)
			);
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'margin_top',
					'title'      => esc_html__( 'Margin Top (px or %)', 'gracey-core' ),
					'group'      => esc_html__( 'Style', 'gracey-core' ),
				)
			);
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'margin_bottom',
					'title'      => esc_html__( 'Margin Bottom (px or %)', 'gracey-core' ),
					'group'      => esc_html__( 'Style', 'gracey-core' ),
				)
			);
		}
		...
		public function render( $options, $content = null ) {
			...
			return gracey_core_get_template_part( 'shortcodes/separator', 'templates/separator', '', $atts );
		}

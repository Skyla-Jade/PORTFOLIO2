...
abstract class GraceyCore_List_Shortcode extends QodeFrameworkShortcode {
	private $post_type;
		...
		if ( empty( $exclude_behavior ) || ( ! empty( $exclude_behavior ) && ! in_array( 'behavior', $exclude_behavior, true ) ) ) {
			$this->set_option(
				array(
					...
					'options'       => gracey_core_get_select_type_options_pool( 'list_behavior', false, $exclude_behavior ),
		...
		if ( empty( $exclude_behavior ) || ( ! empty( $exclude_behavior ) && ! in_array( 'masonry', $exclude_behavior, true ) ) ) {
			$this->set_option(
				array(
					...
					'title'         => esc_html__( 'Image Proportions', 'gracey-core' ),
					'options'       => array(
						''      => esc_html__( 'Original', 'gracey-core' ),
		...
		if ( empty( $exclude_option ) || ( ! empty( $exclude_option ) && ! in_array( 'images_proportion', $exclude_option, true ) ) ) {
			$this->set_option(
				array(
					...
					'default_value' => 'full',
					'title'         => esc_html__( 'Image Proportions', 'gracey-core' ),
					'options'       => gracey_core_get_select_type_options_pool( 'list_image_dimension', false ),
					'group'         => $group,
					'dependency'    => array(
						'show' => array(
							'behavior' => array(
			...
			$this->set_option(
				array(
					...
					'title'       => esc_html__( 'Custom Image Width', 'gracey-core' ),
					'description' => esc_html__( 'Enter image width in px', 'gracey-core' ),
					'group'       => $group,
					'dependency'  => array(
						'show' => array(
			...
			$this->set_option(
				array(
					...
					'title'       => esc_html__( 'Custom Image Height', 'gracey-core' ),
					'description' => esc_html__( 'Enter image height in px', 'gracey-core' ),
					'group'       => $group,
					'dependency'  => array(
						'show' => array(
							'images_proportion' => array(
		...
		if ( empty( $exclude_option ) || ( ! empty( $exclude_option ) && ! in_array( 'columns', $exclude_option, true ) ) ) {
			$this->set_option(
				array(
					...
					'options'       => gracey_core_get_select_type_options_pool( 'columns_number', true, $exclude_columns, $include_columns ),
					'default_value' => '3',
					'group'         => $group,
					'dependency'    => array(
						'hide' => array(
		...
		if ( empty( $exclude_behavior ) || ( ! empty( $exclude_behavior ) && ! in_array( 'justified-gallery', $exclude_behavior, true ) ) ) {
			$this->set_option(
				array(
					...
					'title'      => esc_html__( 'Row Height', 'gracey-core' ),
					'dependency' => array(
						'show' => array(
							'behavior' => array(
								'values'        => 'justified-gallery',
			...
			$this->set_option(
				array(
					...
					'title'      => esc_html__( 'Max Row Height', 'gracey-core' ),
					'dependency' => array(
						'show' => array(
							'behavior' => array(
								'values'        => 'justified-gallery',
			...
			$this->set_option(
				array(
					...
					'title'      => esc_html__( 'Last Row Treshold', 'gracey-core' ),
					'dependency' => array(
						'show' => array(
							'behavior' => array(
								'values'        => 'justified-gallery',
	...
	public function map_slider_options( $params = array() ) {
        ...
        $this->set_option( array(
            'field_type' => 'select',
            'name'       => 'slider_center',
            'title'      => esc_html__( 'Center Slider', 'gracey-core' ),
        ...
        $this->set_option( array(
            'field_type' => 'select',
            'name'       => 'slider_width',
            'title'      => esc_html__( 'Enable Slider Auto Width', 'gracey-core' ),
		...
		$this->set_option(
			array(
				'field_type'  => 'text',
				'name'        => 'slider_speed',
				'title'       => esc_html__( 'Slide Duration', 'gracey-core' ),
				'description' => esc_html__( 'Default value is 5000 (ms)', 'gracey-core' ),
				'dependency'  => $dependency,
				'group'       => $group,
			)
		);
		$this->set_option(
			array(
				...
				'title'       => esc_html__( 'Slide Animation Duration', 'gracey-core' ),
				'description' => esc_html__( 'Speed of slide animation in milliseconds. Default value is 800.', 'gracey-core' ),
				'dependency'  => $dependency,
				'group'       => $group,
			)
		);
	...
	public function get_list_item_image_dimension( $atts ) {
		...
		if ( ! empty( $atts['behavior'] ) && 'masonry' === $atts['behavior'] && ! empty( $atts['masonry_images_proportion'] ) && 'fixed' === $atts['masonry_images_proportion'] ) {
			...
			$image_dimension              = gracey_core_get_custom_image_size_meta( 'meta-box', $masonry_image_dimension_name, get_the_ID() );
		}

		if ( ! empty( $atts['behavior'] ) && in_array( $atts['behavior'], array( 'columns', 'slider' ), true ) ) {
			$image_dimension = array(
				'size'  => $atts['images_proportion'],
				'class' => gracey_core_get_custom_image_size_class_name( $atts['images_proportion'] ),
			);
		}
	...
	public function get_slider_data( $atts, $include = array() ) {
        ...
        $data['direction']      = isset( $atts['slider_direction'] ) ? $atts['slider_direction'] : 'horizontal';
        $data['sliderScroll']   = isset( $atts['slider_mousewheel'] ) && $atts['slider_mousewheel'] == 'yes' ? true : false;
		$data['spaceBetween']   = isset( $atts['space'] ) ? ( gracey_core_get_space_value( $atts['space'] ) * 2 ) : 0; // double half space for slider
		...
		$data['effect']         = isset( $atts['slider_effect'] ) ? $atts['slider_effect'] : 'slide';
		$data['loopedSlides']   = isset( $atts['slider_looped_slides'] ) ? $atts['slider_looped_slides']: '';
	...
	public function map_query_options( $params = array() ) {
		$group                = isset( $params['group'] ) ? $params['group'] : esc_html__( 'Query', 'gracey-core' );
		...
		$this->set_option(
			array(
				...
				'options'       => gracey_core_get_select_type_options_pool( 'order_by', true, $exclude_order_by, $include_order_by ),
				'default_value' => 'date',
				'group'         => $group,
			)
		);
		...
		if ( empty( $exclude_option ) || ( ! empty( $exclude_option ) && ! in_array( 'additional_params', $exclude_option, true ) ) ) {
			...
			do_action( 'gracey_core_action_map_query_options_before_additional', $group );

			$additional_params = apply_filters(
				'gracey_core_filter_map_additional_query_params',
				array(
					''       => esc_html__( 'No', 'gracey-core' ),
					'id'     => esc_html__( 'Post IDs', 'gracey-core' ),
					'tax'    => esc_html__( 'Taxonomy Slug', 'gracey-core' ),
					'author' => esc_html__( 'Author Name', 'gracey-core' ),
				),
				$this->get_base()
			);
			...
			$this->set_option(
				array(
					...
					'title'      => esc_html__( 'Additional Params', 'gracey-core' ),
			...
			if ( ! empty( $taxonomies_formatted ) ) {
				$this->set_option(
					array(
						'field_type' => 'select',
						'name'       => 'tax',
						'title'      => esc_html__( 'Taxonomy', 'gracey-core' ),
				...
				$this->set_option(
					array(
						'field_type' => 'text',
						'name'       => 'tax_slug',
						'title'      => esc_html__( 'Taxonomy Slug', 'gracey-core' ),
						'group'      => $group,
						'dependency' => array(
							'show' => array(
								'additional_params' => array(
				...
				$this->set_option(
					array(
						'field_type'  => 'text',
						'name'        => 'tax__in',
						'title'       => esc_html__( 'Taxonomy IDs', 'gracey-core' ),
						'description' => esc_html__( 'Separate taxonomy IDs with commas', 'gracey-core' ),
						'group'       => $group,
						'dependency'  => array(
							'show' => array(
								'additional_params' => array(
			...
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'author_slug',
					'title'      => esc_html__( 'Author Slug', 'gracey-core' ),
					'group'      => $group,
					'dependency' => array(
						'show' => array(
							'additional_params' => array(
			...
			do_action( 'gracey_core_action_map_query_options_after_additional', $group );
		}
	}
	...
	public function get_additional_query_args( $atts ) {
		...
		$args = apply_filters( 'gracey_core_filter_additional_query_args', $args, $atts, $this->get_post_type() );
		...
		if ( empty( $exclude_option ) || ( ! empty( $exclude_option ) && ! in_array( 'text_transform', $exclude_option, true ) ) ) {
			$this->set_option(
				array(
					'field_type' => 'select',
					'name'       => 'text_transform',
					'title'      => esc_html__( 'Title Text Transform', 'gracey-core' ),
	...
	public function map_additional_options( $params = array() ) {
		$group_name         = esc_html__( 'Additional', 'gracey-core' );
		...
		if ( ! $exclude_filter ) {
			$this->set_option(
				array(
					'field_type'    => 'select',
					'name'          => 'enable_filter',
					'title'         => esc_html__( 'Enable Filter', 'gracey-core' ),
					'description'   => esc_html__( 'Enabling this option will show categories filter above list', 'gracey-core' ),
			...
			do_action( 'gracey_core_action_map_additional_options_after_filter', $group_name );
		}
		...
		if ( ! $exclude_pagination ) {
			...
			$this->set_option(
				array(
					...
					'title'         => esc_html__( 'Load More Top Margin (px or %)', 'gracey-core' ),
					'default_value' => '',
					'group'         => $group_name,
					'dependency'    => array(
						'show' => array(
			...
			do_action( 'gracey_core_action_map_additional_options_after_pagination', $group_name );
		}
	}
	...
	public function register_list_scripts() {
		...
		$list_scripts = apply_filters( 'gracey_core_filter_register_list_shortcode_scripts', isset( $scripts ) ? $scripts : array() );

		$this->set_scripts( $list_scripts );
	}

	public function load_assets() {
		do_action( 'gracey_core_action_list_shortcodes_load_assets', $this->get_atts() );
	}

...
if ( ! function_exists( 'gracey_core_add_image_with_text_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function gracey_core_add_image_with_text_shortcode( $shortcodes ) {
		$shortcodes[] = 'GraceyCore_Image_With_Text_Shortcode';

		return $shortcodes;
	}

	add_filter( 'gracey_core_filter_register_shortcodes', 'gracey_core_add_image_with_text_shortcode' );
}

if ( class_exists( 'GraceyCore_Shortcode' ) ) {
	class GraceyCore_Image_With_Text_Shortcode extends GraceyCore_Shortcode {

		public function __construct() {
			$this->set_layouts( apply_filters( 'gracey_core_filter_image_with_text_layouts', array() ) );
			$this->set_extra_options( apply_filters( 'gracey_core_filter_image_with_text_extra_options', array() ) );

			parent::__construct();
		}

		public function map_shortcode() {
			$this->set_shortcode_path( GRACEY_CORE_SHORTCODES_URL_PATH . '/image-with-text' );
			$this->set_base( 'gracey_core_image_with_text' );
			$this->set_name( esc_html__( 'Image With Text', 'gracey-core' ) );
            ...
            $this->set_option(
                array(
                    'field_type'  => 'select',
                    'name'        => 'enable_shadow',
                    'title'       => esc_html__( 'Enable Shadow', 'gracey-core' ),
            ...
            $this->set_option(
                array(
                    ...
                    'title'      => esc_html__( 'Title Font Size', 'gracey-core' ),
                    'group'      => esc_html__( 'Content', 'gracey-core' ),
                )
            );
			$this->set_option(
		...
		public static function call_shortcode( $params ) {
			$html = qode_framework_call_shortcode( 'gracey_core_image_with_text', $params );
			$html = str_replace( "\n", '', $html );
		...
		public function render( $options, $content = null ) {
			...
			return gracey_core_get_template_part( 'shortcodes/image-with-text', 'variations/' . $atts['layout'] . '/templates/' . $atts['layout'], '', $atts );
		}

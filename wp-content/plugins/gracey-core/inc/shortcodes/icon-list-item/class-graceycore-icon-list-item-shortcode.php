...
if ( ! function_exists( 'gracey_core_add_icon_list_item_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function gracey_core_add_icon_list_item_shortcode( $shortcodes ) {
		$shortcodes[] = 'GraceyCore_Icon_List_Item_Shortcode';

		return $shortcodes;
	}

	add_filter( 'gracey_core_filter_register_shortcodes', 'gracey_core_add_icon_list_item_shortcode' );
}

if ( class_exists( 'GraceyCore_Shortcode' ) ) {
	class GraceyCore_Icon_List_Item_Shortcode extends GraceyCore_Shortcode {

		public function map_shortcode() {
			$this->set_shortcode_path( GRACEY_CORE_SHORTCODES_URL_PATH . '/icon-list-item' );
			$this->set_base( 'gracey_core_icon_list_item' );
			$this->set_name( esc_html__( 'Icon List Item', 'gracey-core' ) );
			$this->set_description( esc_html__( 'Shortcode that adds icon list item element', 'gracey-core' ) );
			$this->set_category( esc_html__( 'Gracey Core', 'gracey-core' ) );
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'item_margin_bottom',
					'title'      => esc_html__( 'Item Margin Bottom', 'gracey-core' ),
				)
			);
			$this->set_option(
				array(
			...
			$this->set_option(
				array(
					...
					'group'      => esc_html__( 'Title', 'gracey-core' ),
				)
			);
			$this->set_option(
				array(
		...
		public function render( $options, $content = null ) {
			...
			return gracey_core_get_template_part( 'shortcodes/icon-list-item', 'templates/icon-list-item', '', $atts );
		}

...
if ( ! function_exists( 'gracey_core_add_icon_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function gracey_core_add_icon_shortcode( $shortcodes ) {
		$shortcodes[] = 'GraceyCore_Icon_Shortcode';

		return $shortcodes;
	}

	add_filter( 'gracey_core_filter_register_shortcodes', 'gracey_core_add_icon_shortcode', 8 );
}

if ( class_exists( 'GraceyCore_Shortcode' ) ) {
	class GraceyCore_Icon_Shortcode extends GraceyCore_Shortcode {

		public function map_shortcode() {
			$this->set_shortcode_path( GRACEY_CORE_SHORTCODES_URL_PATH . '/icon' );
			$this->set_base( 'gracey_core_icon' );
			$this->set_name( esc_html__( 'Icon', 'gracey-core' ) );
			$this->set_description( esc_html__( 'Shortcode that adds icon element', 'gracey-core' ) );
			...
			$this->set_option(
				array(
					...
					'options'    => array(
						...
						'tiny'    => esc_html__( 'Tiny', 'gracey-core' ),
						'small'   => esc_html__( 'Small', 'gracey-core' ),
						'medium'  => esc_html__( 'Medium', 'gracey-core' ),
						'large'   => esc_html__( 'Large', 'gracey-core' ),
						'huge'    => esc_html__( 'Huge', 'gracey-core' ),
					),
				)
			);
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'custom_size',
					'title'      => esc_html__( 'Custom Size', 'gracey-core' ),
				)
			);
			$this->set_option(
				array(
			...
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'shape_size',
					'title'      => esc_html__( 'Shape Size', 'gracey-core' ),
					'dependency' => array(
						'show' => array(
							'icon_layout' => array(
			...
			$this->set_option(
				array(
					...
					'title'      => esc_html__( 'Vertical Offset', 'gracey-core' ),
				)
			);
			$this->set_option(
				array(
		...
		public static function call_shortcode( $params ) {
			$html = qode_framework_call_shortcode( 'gracey_core_icon', $params );
			$html = str_replace( "\n", '', $html );
		...
		public function render( $options, $content = null ) {
			...
			return gracey_core_get_template_part( 'shortcodes/icon', 'templates/icon', '', $atts );
		}

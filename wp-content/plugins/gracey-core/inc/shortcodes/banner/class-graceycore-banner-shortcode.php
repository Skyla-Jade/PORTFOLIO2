...
if ( ! function_exists( 'gracey_core_add_banner_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function gracey_core_add_banner_shortcode( $shortcodes ) {
		$shortcodes[] = 'GraceyCore_Banner_Shortcode';

		return $shortcodes;
	}

	add_filter( 'gracey_core_filter_register_shortcodes', 'gracey_core_add_banner_shortcode' );
}

if ( class_exists( 'GraceyCore_Shortcode' ) ) {
	class GraceyCore_Banner_Shortcode extends GraceyCore_Shortcode {

		public function __construct() {
			$this->set_layouts( apply_filters( 'gracey_core_filter_banner_layouts', array() ) );
			$this->set_extra_options( apply_filters( 'gracey_core_filter_banner_extra_options', array() ) );

			parent::__construct();
		}

		public function map_shortcode() {
			$this->set_shortcode_path( GRACEY_CORE_SHORTCODES_URL_PATH . '/banner' );
			$this->set_base( 'gracey_core_banner' );
			$this->set_name( esc_html__( 'Banner', 'gracey-core' ) );
			$this->set_description( esc_html__( 'Shortcode that adds banner element', 'gracey-core' ) );
			...
			$this->set_option(
				array(
					'field_type'    => 'select',
					'name'          => 'subtitle_tag',
					'title'         => esc_html__( 'Subtitle Tag', 'gracey-core' ),
					'options'       => gracey_core_get_select_type_options_pool( 'title_tag' ),
					'default_value' => 'h5',
					'group'         => esc_html__( 'Content', 'gracey-core' ),
				)
			...
			$this->set_option(
				array(
					'field_type' => 'color',
					'name'       => 'subtitle_color',
					'title'      => esc_html__( 'Subtitle Color', 'gracey-core' ),
					'group'      => esc_html__( 'Content', 'gracey-core' ),
				)
			);
			$this->set_option(
				array(
					...
					'title'      => esc_html__( 'Subtitle Margin Top', 'gracey-core' ),
					'group'      => esc_html__( 'Content', 'gracey-core' ),
				)
			);
			$this->set_option(
		...
		public function render( $options, $content = null ) {
			...
			return gracey_core_get_template_part( 'shortcodes/banner', 'variations/' . $atts['layout'] . '/templates/' . $atts['layout'], '', $atts );
		}

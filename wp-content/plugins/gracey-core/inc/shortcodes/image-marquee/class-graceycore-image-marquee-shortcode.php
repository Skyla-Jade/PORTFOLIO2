...
if ( ! function_exists( 'gracey_core_add_image_marquee_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function gracey_core_add_image_marquee_shortcode( $shortcodes ) {
		$shortcodes[] = 'GraceyCore_Image_Marquee_Shortcode';

		return $shortcodes;
	}

	add_filter( 'gracey_core_filter_register_shortcodes', 'gracey_core_add_image_marquee_shortcode' );
}

if ( class_exists( 'GraceyCore_Shortcode' ) ) {
	class GraceyCore_Image_Marquee_Shortcode extends GraceyCore_Shortcode {

		public function __construct() {
			$this->set_layouts( apply_filters( 'gracey_core_filter_image_marquee_layouts', array() ) );
			$this->set_extra_options( apply_filters( 'gracey_core_filter_image_marquee_extra_options', array() ) );

			parent::__construct();
		}

		public function map_shortcode() {
			$this->set_shortcode_path( GRACEY_CORE_SHORTCODES_URL_PATH . '/image-marquee' );
			$this->set_base( 'gracey_core_image_marquee' );
			$this->set_name( esc_html__( 'Image Marquee', 'gracey-core' ) );
			$this->set_description( esc_html__( 'Shortcode that adds Image Marquee element', 'gracey-core' ) );
			...
			$this->set_option(
				array(
					'field_type'    => 'text',
					'name'          => 'duration',
					'title'         => esc_html__( 'Animation Duration (Seconds)', 'gracey-core' ),
					'default_value' => '20',
				)
			);
			$this->set_option(
		...
		public function render( $options, $content = null ) {
			...
			return gracey_core_get_template_part( 'shortcodes/image-marquee', 'variations/' . $atts['layout'] . '/templates/' . $atts['layout'], '', $atts );
		}

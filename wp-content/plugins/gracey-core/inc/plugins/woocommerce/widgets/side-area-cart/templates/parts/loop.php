		...
		if ( $_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters( 'woocommerce_cart_item_visible', true, $cart_item, $cart_item_key ) ) {
			...
			<div class="qodef-m-item qodef-e">
				...
				<div class="qodef-e-content">
					...
					<p class="qodef-e-quantity"><?php echo sprintf( esc_html__( 'Quantity: %s', 'gracey-core' ), esc_attr( $cart_item['quantity'] ) ); ?></p>
					<?php echo sprintf( '<a href="%s" class="qodef-e-remove remove" title="%s">%s</a>', esc_url( wc_get_cart_remove_url( $cart_item_key ) ), esc_attr__( 'Remove this item', 'gracey-core' ), qode_framework_icons()->get_specific_icon_from_pack( 'close', 'elegant-icons' ) ); ?>
				</div>
			</div>
			<?php
		}

...
class GraceyCore_Titles {
	...
	public function __construct() {
		...
		add_filter( 'gracey_filter_page_title_classes', array( $this, 'add_page_title_classes' ) );

		// Add title area inline styles
		add_filter( 'gracey_filter_add_inline_style', array( $this, 'add_inline_styles' ) );
	}

	/**
	 * @return GraceyCore_Titles
	 */
	...
	function include_layouts() {
		...
		foreach ( glob( GRACEY_CORE_INC_PATH . '/title/dashboard/*/*.php' ) as $admin ) {
			include_once $admin;
		}

		foreach ( glob( GRACEY_CORE_INC_PATH . '/title/layouts/*/include.php' ) as $layout ) {
			include_once $layout;
		}
	}

	function set_variables() {
		$layout_meta = gracey_core_get_post_value_through_levels( 'qodef_title_layout' );
		$layouts     = apply_filters( 'gracey_core_filter_register_title_layouts', $layouts = array() );
	...
	function render_template() {
		...
		if ( ! empty( $title_object ) ) {
			$template_hook = $title_object->overriding_whole_title ? 'gracey_filter_title_template' : 'gracey_filter_title_content_template';
	...
	function add_page_title_classes( $classes ) {
		$layout             = gracey_core_get_post_value_through_levels( 'qodef_title_layout' );
		$text_alignment     = gracey_core_get_post_value_through_levels( 'qodef_page_title_text_alignment' );
		$vertical_alignment = gracey_core_get_post_value_through_levels( 'qodef_page_title_vertical_text_alignment' );
		$image_params       = gracey_core_get_page_title_image_params();
	...
	function add_inline_styles( $style ) {
		...
		$height                   = gracey_core_get_post_value_through_levels( 'qodef_page_title_height' );
		$height_mobile            = gracey_core_get_post_value_through_levels( 'qodef_page_title_height_on_smaller_screens' );
        $side_padding             = gracey_core_get_post_value_through_levels( 'qodef_page_title_side_padding' );
		$title_padding            = apply_filters( 'gracey_core_filter_title_padding', 0 );
		$title_padding_mobile     = apply_filters( 'gracey_core_filter_title_padding_mobile', 0 );
		$title_vertical_alignment = gracey_core_get_post_value_through_levels( 'qodef_page_title_vertical_text_alignment' );
		$background_color         = gracey_core_get_post_value_through_levels( 'qodef_page_title_background_color' );
		$image_params             = gracey_core_get_page_title_image_params();
        ...
        if ( ! empty( $inner_styles ) ) {
            $style .= qode_framework_dynamic_style( '.qodef-page-title .qodef-m-content.qodef-content-full-width', $inner_styles );
        }
		...
		$title_color = gracey_core_get_post_value_through_levels( 'qodef_page_title_color' );
...
GraceyCore_Titles::get_instance();
